-- Script SQL per aggiungere le tabelle necessarie per calendario e classifica
-- Eseguire questo script in Supabase SQL Editor

-- Prima verifichiamo che le tabelle base esistano
...
-- Ora creiamo le nuove tabelle per il calendario
...
CREATE INDEX IF NOT EXISTS idx_matchdays_competition_id ON matchdays(competition_id);
...
CREATE INDEX IF NOT EXISTS idx_standings_points ON standings(points DESC, goal_difference DESC);
...
/*
INSERT INTO matchdays (competition_id, round_number, date) VALUES
    ((SELECT id FROM competitions LIMIT 1), 1, '2024-01-15'),
    ((SELECT id FROM competitions LIMIT 1), 2, '2024-01-22');
*/
